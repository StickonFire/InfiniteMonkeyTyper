...
MonkeyTyper::MonkeyTyper(MonkeyTyper &&other): query(other.query), rng(std::move(other.rng)), seed(other.seed), id(other.id), packetSize(other.packetSize), currentSpot(other.currentSpot), guessStream(other.guessStream),
...
enum Status MonkeyTyper::moveStream(int charsMoved){
    ...
    for(int packet_spot = 0; packet_spot < charsMoved; packet_spot++){
        ...
        packetCorrespondingQuery[packet_spot] = query[prevMax];
        ...
        if(currentMax == prevMax+1){
            ...
            packetCorrespondingQuery[packet_spot] = query[currentMax]-1;
        }
        else {
...
ListInfo MonkeyTyper::listInfo(){
    ...
    if(packetBestGuessLocation.size() > 0){
        if(complete())
            currentLocation = query.size();
        else
            currentLocation = packetBestGuessLocation.back();
    } 
